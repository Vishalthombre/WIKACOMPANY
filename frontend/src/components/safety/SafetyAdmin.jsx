                ...
                <>
                    ...
                    <div className="desktop-view">
                        <div style={styles.tableCard}>
                            <table style={styles.table}>
                                ...
                                <tbody>
                                    {tickets.map(t => (
                                        <tr key={t.TicketID} style={styles.trBody} onClick={() => setSelectedTicket(t)}>
                                            <td style={styles.td}>#{t.TicketID}</td>
                                            <td style={styles.td}>{new Date(t.CreatedAt).toLocaleDateString()}</td>
                                            <td style={{...styles.td, color:'#e11d48', fontWeight:'600'}}>{t.Keyword}</td>
                                            <td style={styles.td}>
                                                {t.BuildingName}
                                            ...
                                            <td style={styles.td}><button style={styles.detailsBtn}>View Details</button></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                    ...
                    <div className="mobile-view">
                        <div style={styles.grid}>
                            {tickets.map(t => (
                                <div key={t.TicketID} style={styles.mobileCard} onClick={() => setSelectedTicket(t)}>
                                    ...
                                    <div style={styles.cardBody}>
                                        ...
                                        <p style={{margin:'0 0 10px', fontSize:'0.9rem', color:'#475569'}}>{t.Description}</p>
                                        <div style={{fontSize:'0.85rem', color:'#64748b'}}>📍 {t.BuildingName} • {t.AreaName || 'N/A'}</div>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div style={styles.detailModal} onClick={e => e.stopPropagation()}>
                        ...
                        <div style={styles.detailBody}>
                            ...
                            <div style={styles.detailRow}>
                                ...
                                <span>{selectedTicket.BuildingName} &gt; {selectedTicket.AreaName}</span>
                            </div>
                            <div style={styles.detailRow}>
                        ...
                        <div style={styles.detailFooter}>
                            <button onClick={(e) => handleDelete(e, selectedTicket.TicketID)} style={styles.deleteBtn}>🗑️ Delete Ticket</button>
...
const styles = {
    ...
    headerRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px', paddingBottom: '15px', borderBottom: '1px solid #e2e8f0', flexWrap: 'wrap', gap: '15px' },
    header: { color: '#1e293b', fontSize: '1.6rem', margin: 0, fontWeight:'700' },
    subHeader: { margin: '5px 0 0', color: '#64748b', fontSize: '0.9rem' },
    configBtn: { background: '#1e293b', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' },
    ...
    viewPhotoBtn: { width: '100%', padding: '12px', background: '#f1f5f9', border: '1px solid #cbd5e0', borderRadius: '8px', color: '#334155', fontWeight: '600', fontSize: '0.95rem', cursor: 'pointer', textAlign: 'center', transition: 'background 0.2s', display: 'block' },
